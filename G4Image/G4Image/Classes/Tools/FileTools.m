...
/**
 ...
 @param accout 需要保存的帐号
 @param key 保存使用的key
 */
+ (void)saveAccout:(id)accout forKey:(NSString *)key {
    
    [[NSUserDefaults standardUserDefaults] setObject:accout forKey:key];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
+ (NSString *)getFileSizeStr:(NSInteger)totalSize {
    ...
    if (totalSize > 1024 * 1024) {  // 1024
        ...
        sizeString = [NSString stringWithFormat:@"%zdB", totalSize];
    }
    return sizeString;
}
